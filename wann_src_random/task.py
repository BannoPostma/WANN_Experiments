...
# -- 'Weight Agnostic Network' evaluation -------------------------------- -- #
  def setWeightsRandom(self, wVec):
    ...
    wMat = np.copy(cMat) * np.random.uniform(-2,2,size=(dim,dim)) 
    return wMat
  ...
  def getDistFitness(self, wVec, aVec, hyp, \
    ...
    for iRep in range(nRep):
      for iVal in range(nVals):
        wMat = self.setWeightsRandom(wVec)
        if seed == -1:
    ...
    if returnVals is True:
      return np.mean(reward,axis=0), np.array([1.0]*nVals)
