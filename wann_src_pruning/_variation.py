...
def mutRemNode(self, connG, nodeG, innov, gen):
  """Add new node to genome
  ...
  if len (np.where (nodeG[1,:]==3)[0])==0:
    return connG,nodeG, innov

  nodeToDelete = np.random.choice (np.where (nodeG[1,:]==3)[0])
  # Add connections to and from new node
  # -- Effort is taken to minimize disruption from node addition:
  # The weight to the node is set to 1, the weight from is set to the original 
  # weight. With a near linear activation function the change in performance 
  # should be minimal.
  def new_connection(src, dest, connG, innov):
  ...
  nodeId = nodeG[0,nodeToDelete]
  inConnections = np.where (connG [2,:]==nodeId)
  outConnections = np.where (connG [1,:]==nodeId)

  inNodes = connG[1,inConnections].flatten ()
  outNodes = connG[2,outConnections].flatten ()

  for s in inNodes:
    for d in outNodes:
      connG, innov = new_connection(s,d,connG,innov)

  
  connG=np.delete (connG, np.c_[inConnections,outConnections],axis=1)
  nodeG=np.delete (nodeG,[nodeToDelete],axis=1)
...
def mutRemoveConn(self, connG, nodeG, innov, gen):
  """Add new connection to genome.
  To avoid creating recurrent connections all nodes are first sorted into
  layers, connections are then only created from nodes to nodes of the same or
  later layers.
  ...
  """
  connections = np.random.permutation(connG.shape [-1])
  for c in connections:
    if len(np.where (connG[2,:] == connG[2,c])) == 1:
      continue
    if nodeG[np.where (nodeG [0,:]==connG[1,c])] [1,:] == 1 or len(np.where (connG[1,:] == connG[1,c])) > 1:
      newInnov = np.hstack((connG [:,c][0:3].flatten(), -1, gen))
      innov = np.hstack((innov,newInnov[:,None]))
      connG = np.delete (connG,[c],axis=1)
      break
...
def topoMutate(self,child,innov,gen):
  ...
  topoRoulette = np.array((p['prob_addConn'], p['prob_remConn'], p['prob_addNode'], p['prob_remNode'], \
  ...
  # Add Connection
    ...
    connG, innov = self.mutRemoveConn(connG, nodeG, innov, gen)  
  ...
  # Add Node
    ...
    connG, nodeG, innov = self.mutRemNode(connG, nodeG, innov, gen)
